...
When `kien-cover-letter` passes a formal letter to Humanizer, the facts and stories are locked first. Humanizer then applies the rewrite and audit protocol, including checks for false agency, vague claims, adverb emphasis, portability, and minimum effective editing. It does not add errors or chase a detector score.
...
When you paste text, Humanizer uses three passes: it locks the meaning and calibrates to a writing sample, rewrites for voice and specificity, removes AI patterns, and then audits what still sounds generated before making the final edit. It protects facts, terminology, citations, links, and other locked items; long text is handled in 150–300-word sections before tone is unified. It shows the first rewrite, an audit, a change summary, a verification checklist, an optional five-dimension quality score, and the final version. Point it at a file and it changes only the prose, leaving code, data, frontmatter, and link targets alone. Personal writing keeps the writer's opinions and quirks. Technical and reference prose stays neutral and plain. It does not add fake mistakes or chase a detector score.

### The three-pass plan

1. **Brief and meaning lock.** Record audience, purpose, format, tone, length, protected facts, keywords, entities, citations, links, and terminology. Keep the event, choice, action, and consequence in a real story. Ask for missing detail instead of inventing it; for long text, work section by section.
2. **Voice and pattern pass.** Match the writer's sample for cadence, vocabulary, paragraph openings, transitions, punctuation, and deliberate quirks. Add supported judgement, constraints, trade-offs, and a reason the writer needed to say this. Then remove staging, inflated claims, vague authority, filler, decorative formatting, tidy contrasts, repeated openings, and mechanical rhythm. Use direct words and concrete details. Do not run a synonym-only polish pass.
3. **Audit and final rewrite.** Read aloud and ask, "What still gives this away as AI?" Fix only genuine remaining tells, compare the result with the meaning lock, and return a change summary plus verification checklist. For substantial prose, optionally score directness, rhythm, trust, authenticity, and density from 1–10; below 35/50 means one targeted revision. For formal applications, check punctuation, watched words, sentence lengths, passive constructions, and factual anchors, and run the Signal I audit from `harshaneel/humanize` once more.
...
- **3.3.0** - Added an optional five-dimension quality score, confidence guidance for too few voice samples, and an explicit boundary that the score is a revision aid rather than a detector or authorship judgment.
- **3.2.0** - Added protected keywords, entities, citations, links, length and chunk controls, supported judgement and trade-off checks, change summaries, verification checklists, and a stop condition for thin or templated source material, based on the Lynote handbook.
- **3.1.0** - Made the three-pass workflow explicit: meaning lock and voice calibration, separate voice and pattern editing, then a quoted AI-tell audit with a final fact check. Added a no-detector-chasing guardrail and a story-specific lead and decision check.
- **3.0.1** - Added a formal application-letter mode with fact locking, stop-slop checks for false agency and vague declaratives, literal rhythm and punctuation counts, and one Signal I audit loop. Formal applications do not receive detector-driven typos or fake disfluencies.
